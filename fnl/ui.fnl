  ...
  (let [Menu (require :nui.menu)
    ...
    (do
      ...
      (let [current-node nil
            ...
            get-keymap-args (fn [] {:node current-node : menu})]
        (do
          (each [i2 m (ipairs mappings)]
            (do
              (menu:map :n (. m 1) ((. m 2) (get-keymap-args)))
              (vim.api.nvim_buf_set_lines popup.bufnr i2 -1 false
                                          [(.. (. m 1) "\\t" m.desc)])))
...
{: create-input : create-textbox : create-menu-with-key : create-menu}
