...
(local bindings [
  (leader! [:<leader> (module-call! :telescope.builtin :find_files) { "desc" "Browse Files" } ])
  (leader! [:/ (module-call! :telescope.builtin :live_grep) { "desc" "Live Grep" } ])
  (leader! [:? (module-call! :telescope.builtin :keymaps) { "desc" "Keymaps" } ])
  ...
  (leader-group! "git" [
                        ...
                        [:b (module-call! :nvit :view_branches) { "desc" "Git Branches" }]
                        [:f (module-call! :telescope.builtin :git_files) { "desc" "Git Files"}]
                        [:s (module-call! :telescope-builtin :git_status) { "desc" "Git Status" }]
                        [:S (module-call! :telescope-builtin :git_stash) { "desc" "Git Stashes" }]
                 ])

  (leader-group! "gitlab" [
                           [:m (module-call! :nvit :view_merge_requests) { "desc" "Merge Requests" }]
                           [:r (module-call! :nvit :view_reviews) { "desc" "Review Requests" }]
                           [:n (module-call! :nvit :view_notifications) { "desc" "Notifications" }]
                           ] :gl)
  ...
  (leader-group! "buffers" [
                            ...
                            [:N (cmd! "Buffer Previous") { :desc "Previous Buffer" }]
                            [:p (cmd! "BufferPick") { :desc "Pick Buffer" }]
                            [:d (cmd! "BufferClose") { :desc "Close Buffer" }]
                            ])

  (leader! [:r (cmd! "Telescope registers") { :desc "View Registers" }])
  (leader! [:f (cmd! "Neotree position=float") { :desc "View Directory" }])

  (leader! [:% ":%s/\\<<C-r><C-w>\\>/<C-r><C-w>/gI<Left><Left><Left>" { :desc "Find/Replace symbol" }])
])
...
(fn add-mapping [binding]
  (case binding
    (where
      [key _ opts] (?. opts :expansions)) (do
        (table.insert mappings (doto {} (tset 1 key) (tset :group (. opts :group))))
        (each [_ binding2 (pairs (. opts :expansions))]
          (add-mapping binding2)))
    [key callback opts]
      (table.insert mappings (do (tset opts 1 key) (tset opts 2 callback) opts))))

(each [_ binding (pairs bindings)] (add-mapping binding))
(do ((. (require :which-key) :add) mappings))
