...
-- Convert menu item to NUI menu item
local function to_nui_menu_item(item)
...
-- Create centered float window configuration
...
-- Check if menu needs additional popup
...
-- Create a textbox for multiline input
...
-- Check if input needs additional popup
local function input_needs_popup(options)
  return options.desc or options.mappings
end

-- Create an input field with optional description popup
...
-- Calculate default column position for pins
...
-- Calculate default row position for pins
...
-- Remove a pin and optionally rerender others
local function remove_pin(title, rerender)
  local function close_pin(pin)
...
-- Create a pinned floating window
